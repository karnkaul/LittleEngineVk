...
auto EngineStats::draw_to(OpenWindow /*w*/) -> void {
	...
	assert(m_frame_time_offset < m_frame_times.size());
	m_frame_times[m_frame_time_offset] = std::chrono::duration<float, std::milli>{stats.frame.time}.count();
	...
	if (auto tn = TreeNode{"frame"}) {
		...
		auto const overlay_text = FixedString{"{:.1f}ms", m_frame_times[m_frame_time_offset]};
		ImGui::PlotLines("time", m_frame_times.data(), static_cast<int>(m_frame_times.size()), static_cast<int>(m_frame_time_offset), overlay_text.c_str(),
						 0.1f, 30.0f, {0.0f, 50.0f});
...
auto EngineStats::set_frame_time_count(std::size_t const count) -> void {
	if (count == 0) { return; }
	m_frame_times.resize(count);
	if (m_frame_time_offset >= m_frame_times.size()) { m_frame_time_offset = m_frame_times.size() - 1; }
}
} // namespace le::imcpp
