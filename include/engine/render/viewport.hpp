...
constexpr Viewport::ScreenRect Viewport::rect() const noexcept {
	return ScreenRect::sizeTL({scale, scale}, topLeft.norm);
}
...
constexpr glm::vec2 Viewport::centre(glm::vec2 coeff) const noexcept {
	return (coeff * topLeft.norm + (size() * 0.5f)) + topLeft.offset;
}
