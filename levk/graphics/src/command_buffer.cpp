...
CommandBuffer::CommandBuffer(vk::CommandBuffer cmd, bool recording) : m_cb(cmd) {
	KASSERT(!Device::default_v(cmd), "Null command buffer!");
...
void CommandBuffer::begin(vk::CommandBufferUsageFlags usage, vk::CommandBufferInheritanceInfo const* inheritance) {
	KASSERT(valid() && !recording() && !rendering(), "Invalid command buffer state");
...
void CommandBuffer::setViewport(vk::Viewport viewport) const {
	KASSERT(recording(), "Command buffer not recording!");
...
void CommandBuffer::endRenderPass() {
	KASSERT(rendering(), "Command buffer not rendering!");
	m_cb.endRenderPass();
	m_flags.reset(Flag::eRendering);
}

void CommandBuffer::end() {
	KASSERT(recording() && !rendering(), "Command buffer not recording!");
	m_cb.end();
