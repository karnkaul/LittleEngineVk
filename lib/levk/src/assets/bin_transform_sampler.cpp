...
void levk::to_bytes(std::vector<std::byte>& out, TransformSampler const& transform_sampler) {
	auto writer = levk::binary::Writer{out};
	out.reserve(out.size() + sizeof(levk::binary::Header) + get_size_bytes(transform_sampler));
	std::visit(WriteBytes{writer}, transform_sampler.sampler);
}

void levk::from_bytes(std::span<std::byte const> bytes, TransformSampler& out) {
	auto reader = levk::binary::Reader{bytes};
	ReadBytes{out}(reader);
}

void levk::to_bytes(std::vector<std::byte>& out, std::span<TransformSampler const> samplers) {
	auto writer = levk::binary::Writer{out};
	for (auto const& sampler : samplers) {
		out.reserve(out.size() + sizeof(levk::binary::Header) + get_size_bytes(sampler));
		std::visit(WriteBytes{writer}, sampler.sampler);
	}
}

void levk::from_bytes(std::span<std::byte const> bytes, std::vector<TransformSampler>& out) {
	auto reader = levk::binary::Reader{bytes};
	ReadBytes2{out}(reader);
}
